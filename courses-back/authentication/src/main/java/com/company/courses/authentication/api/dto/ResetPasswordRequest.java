...
public class ResetPasswordRequest {
    ...
    public @NotBlank(message = "User password cannot be blank") @Size(min = 8, max = 100, message = "User password must be {max} characters or less") @Pattern(regexp = "^(?=.*[A-Za-z0-9@#$%^&+=*]).{8,100}$",
            message = "Password must contain letters, numbers and special characters, and be at least 8 characters long") String getNewPassword1() {
        return newPassword1;
    }

    public @NotBlank(message = "User password cannot be blank") @Size(min = 8, max = 100, message = "User password must be {max} characters or less") @Pattern(regexp = "^(?=.*[A-Za-z0-9@#$%^&+=*]).{8,100}$",
            message = "Password must contain letters, numbers and special characters, and be at least 8 characters long") String getNewPassword2() {
        return newPassword2;
    }
}
